 ...
 *   - Backlog sections: accordion header + expandable task list + "Ещё задачи"
 *
...
/**
 * TaskGroup — section with amber accent line + title + task list.
 * Figma "task-group": column, gap 16px, stretch.
 */
function TaskGroup({ title, tasks }: { title: string; tasks: TaskEntity[] }) {
...
/**
 * BacklogAccordion — collapsible backlog section with "Ещё задачи" button.
 * Figma "backlog-tasks-accordion": header + list + button.
 */
function BacklogAccordion({ tasks }: { tasks: TaskEntity[] }) {
  const [open, setOpen] = useState(false);
  const visibleTasks = open ? tasks : tasks.slice(0, 2);
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-expanded={open}
        aria-label={open ? 'Свернуть бэклог' : 'Развернуть бэклог'}
      >
...
export function StreamView({
  ...
  return (
    ...
    >
      ...
      {backlog.length > 0 && (
        <div className="flex w-full flex-col gap-4">
          <SectionHeader title="Бэклог" />
          <BacklogAccordion tasks={backlog} />
        </div>
      )}

      {/* Done group */}
      {done.length > 0 && (
        <TaskGroup title="Сделано" tasks={done} />
      )}
