...
**Версия:** 0.8.1
**Дата:** август 2026
**Статус:** ⚠️ LEGACY — исторический документ (v0.8.x). Актуальный контракт — Arch 0.9 (`docs/refactor-ai/`).

> ⚠️ **CL-01 (2026-09-03):** инструмент `wait_for_tasks` (§3, §4.10, §7 п.15) и Duty Mode long-poll **удалены** из системы. Пробуждение агентов — `ops_lease` + Realtime broadcast (спеки 12–14). Документ сохранён для истории дизайна; разделы про `wait_for_tasks`, `agent_duty_playbook`, `autonomy_level` не действительны.
...
```typescript
{
  workspace_id:   string,
  agent_name:     string,
  known_task_ids?: string[],  // ACK-дельта: id задач, ОБРАБОТАННЫХ с прошлого
                              // вызова (не вся история!). Хранится на сервере
                              // (agent_duty_state, миграция 056), переживает
                              // компакты. Легаси-клиенты с полной историей
                              // деградируют корректно (всё станет ack'ами)
...
**Server-side duty state (CTX-01/01a, миграция 056):** таблица
...
идентификатора сессии. **Доставка двухфазная (deliver → ack):**
- `delivered` — мягкая пометка, ставится в момент возврата задачи в ответе
  (persist ДО ответа ⇒ при сбое at-least-once). Глушит повторный wake на
  **10 мин**: задача, которую агент не подтвердил (краш сессии, занятость,
  потерянный батч), приходит повторно автоматически — недоставленное
  не теряется;
- `acked` — жёсткая пометка (**24ч**): клиент эхом вернул обработанные id
  через `known_task_ids` (только дельту, не всю историю).
Cap 500 id, брошенные стейты (>7 дней) подчищаются при загрузке. Payload
клиента мал и постоянен по форме, состояние переживает Auto Compact без
реконструкции.
...
2. **`deploy_requests`** (только ключи `autonomy_level='full'`): задача агента
   ...
   из TWA. Агент решает домен: разработка → git-цепочка деплоя; не-разработка →
   ничего (результат уже записан).
3. **`fix_requests`**: задача агента возвращена review → in_progress (доработка)
   — **любой домен**, агент обязан узнать о возврате. Причина возврата — в поле
   `fix_reason` элемента (миграция 051; продублирована в
   `tasks.metadata.last_fix_reason`, читается через get_task_context).

Каждый переход доставляется **ровно один раз на агента** — дедуп маркерными
событиями `agent_events` (`tool='deploy_notify'|'fix_notify'`,
`metadata.history_id` = id записи `task_column_history`). Переживает рестарт
сессии агента.
...
```typescript
{
  ...
  deploy_requests?: TaskPreview[],// одобрены (review→done); full-ключи
  fix_requests?: TaskPreview[],   // возвращены на доработку; любой домен
  waited_ms: number
}
```
...
```
while (true):
  r = wait_for_tasks({ known_task_ids: ack_delta, timeout_sec: 30,
                       poll_seq: prev + 1 })
  if r.status == 'new_tasks':
      для каждой задачи → get_task_context → работа по §7
      ack_delta += обработанные id   # ack — иначе придёт повторно (~10 мин)
...
- Read-only по tasks/квоте; в `agent_events` пишутся ТОЛЬКО маркеры доставки
  `deploy_notify`/`fix_notify` (по одному на фактическую доставку пинка,
  не при каждом опросе).
