...
- [x] CAL-07 CalDAV: перевести синхронизацию Яндекс-календаря на app password #infra !critical @blocked_by:CAL-01
      **Блокер найден экспериментом 2026-09-26 (не догадкой).** OAuth-токен НЕ работает
      ...
      пути `/calendars/<login>/` и `/principals/users/<login>/`, PROPFIND корня.
      REST-альтернатива тоже проверена и закрыта: `calendar.yandex.ru/api/v1/user` → 404,
      `calendar.yandex.net` → TLS UnknownIssuer, `api.calendar.yandex.net` → DNS. Probe
      после проверки удалён. Яндекс CalDAV принимает только **app password**
      (Яндекс ID → Пароли приложений → тип «Календарь»).
      **Миграция `calendar_connection_caldav_credentials`:** `caldav_password_b64 text`
      (шифр AES-256-GCM, INV-17) + CHECK `last_sync_at IS NULL OR caldav_password_b64
      IS NOT NULL` — метка последнего успешного синка не может существовать без пароля.
      **Edge Function (v0.19.0):** крипто вынесено в `encryptSecret`/`decryptSecret`
      (один механизм и для токенов, и для пароля); `syncYandex` восстановлен с Basic
      (логин+app password) и discovery `calendar-home-set` через PROPFIND, чтобы не
      угадывать путь; парсер VEVENT учитывает RFC 5545 folding и unescape `\n`;
      новое действие `set_password` (пароль добавляется без повторного OAuth);
      `caldav_password` также принимается в `connect`; `428 caldav_password_required`
      без пароля, `caldav_auth_failed_401` при неверном; `last_sync_at` обновляется
      **только** при ошибках == 0 (иначе 401 рисовал бы свежесть над пустым календарём);
      ответ больше не врёт «synced successfully» при ошибке.
      **Проверено вживую:** set_password → sync отдаёт `caldav_auth_failed_401` с
      подсказкой; пароль в БД зашифрован (76 байт); `last_sync_at` не обновился.
      Тестовый пароль из БД удалён после проверки.
      **Осталось:** поле ввода app password в UI подключения (сейчас пароль можно
      задать только через API) — см. CAL-08. Без реального пароля пользователя
      наполнение `calendar_events` проверить нельзя.
- [x] CAL-08 CalDAV app password: ввод в UI, шифрование AES-256-GCM, отсечение утечки в `get_connections` #ui !critical
      Пользователь вводит пароль приложения в модалке; пароль шифруется в Edge Function и наружу
      не отдаётся никогда. Yandex CalDAV OAuth-токен не принимает (401 + `Basic realm="CalDAV"`),
      схема нестандартная: discovery principal → home-set → подколлекции `events-*` → GET `.ics`,
      `calendar-query REPORT` не поддерживается. Поля ввода OAuth-кода вводится один раз.
- [x] CAL-09 Часовые пояса: TZID в iCal + локальный ключ дня #bug !critical
      `DTSTART;TZID=Europe/Moscow:...` терял параметры свойства, «плавающее» время читалось в UTC
      рантайма Edge Functions, событие показывалось на 3 часа позже. Разбор вынесен в
      `calendar-sync/ical.ts`; `lib/calendar.localDateKey` — единственный источник ключа дня.
      Регрессия закрыта 7 тестами с `TZ=Europe/Moscow` (на UTC-раннере отказ невидим) + 35 тестов
      парсера. Проверено мутацией: удаление обработки TZID роняет 3 теста.
- [x] CAL-10 Безопасность календаря: неаутентифицированный `calendar-sync`, IDOR в disconnect, подделка OAuth `state` #security !critical
      Guard Edge Function принимал любой `Bearer ` длиннее 10 символов, а секрет, с которым он
      сравнивал, в проекте не существует — функция была публичной (anon-ключ из бандла проходил).
      `disconnect/[provider]` не аутентифицировал и брал `profile_id` из тела. `state` был сырым
      `profile_id`. Исправлено, подтверждено живым запросом: поддельный bearer / anon-ключ /
      без заголовка — 401. HMAC-`state` покрыт 8 тестами.
- [x] CAL-11 Экран календаря: `День` / `Месяц`, часовая ось, полоса недели, карточка события #ui !high
      Вместо мёртвого react-day-picker: пропорциональная ось с блоками по длительности и
      колонками для пересечений, `WeekStrip`, `MonthView` с читаемыми точками, `EventDetailSheet`
      на штатной `BottomSheet` (тап по событию наконец работает, ссылки в описании кликабельны).
      Автосинхронизация при открытии с порогом 15 минут. Удалены `CalendarView`, `ListView`,
      `ThreeDaysView`, `MonthListView` и 3 неиспользуемых режима. Заголовок и иконка приведены к
      20/24/500 как у «Стола» и «Настроек». −1566/+376 строк.
- [x] CAL-12 События на весь день: `is_all_day` вместо UTC-полночи на оси #bug !med
      `VALUE=DATE` хранились как полночь UTC и попадали на ось как 03:00. Миграция 124, парсер
      метит флаг, `AllDayRow` показывает их над осью, карточка — без диапазона времени. 5 тестов.

- [ ] CAL-13 Раскрыть «аккаунт → календари»: имена календарей и цвет по учётке, затем UI #feature !high
      **СТОП — откачено после инцидента (2026-09-26), коммит `f60764a`.**
      События перестали грузиться. Причина — не одна, а три складывающиеся:
      1. `upsertCalendarEvent` берёт `onConflict: 'profile_id,provider,remote_event_id'`,
         а миграция 125 снесла ровно этот уникальный ключ. Каждый upsert события
         падал. **ГЛАВНОЕ:** supabase-js не бросает исключение, а возвращает
         `{ error }`, а функция результат не проверяет и `.throwOnError()` не
         вызывает. Ошибка ушла в никуда: `synced++` отсчитал несуществующие
         записи, `errors` остался пустым, `last_sync_at` обновился как при успехе.
         То есть отчёт о синке врал, и единственным симптомом был пустой календарь.
      2. «Страховка» с ведром `legacy` сработала всё равно: её условие было
         «коллекции зарегистрированы», а не «события реально записаны». Каскад
         `ON DELETE` удалил 7 импортированных событий.
      3. Снос ключа уникальности и перенос его на `calendar_id` — эмоционально
         правильная идея, но менять ключ и писать по нему в одном шаге нельзя.
      **Урок:** перед сменой ключа уникальности в схеме нужно сначала перестать
      его использовать в коде, иначе рассинхрон схемы и кода молча уничтожает
      данные. И `upsert` без проверки `error` — тихая потеря данных в любом слое.
      **Обязательно сделать перед любым следующим резервным слоем данных:**
      `upsertCalendarEvent` должен бросать при ошибке, иначе счётчик `synced`
      и `last_sync_at` будут врать снова.

      **Полезное, что вскрылось тем прогоном:** Яндекс имя календаря отдаёт,
      парсер рабочий с первой попытки:
        /calendars/<login>/events-9527465/  -> "Мои события"
        /calendars/<login>/todos-7699121/   -> "Не забыть"
      Второй — список дел, а не события (VTODO, а не VEVENT), так что при
      возврате к задаче его надо исключать из синка. Различать календари по
      цвету технически возможно.
...
- [ ] DS-07 E2E реального прогона: реальный ключ Drift → задача уходит агенту → `review` + Telegram-апрув #test !high
 ...
 - [x] DUP-01 Воркер `duplicate_check`: потребителя нет, детект дублей не работает #ai !med
       ...
       **ЗАКРЫТО 2026-09-26 (миграции 122 + 123, bot-notify v47).**
       · `process_duplicate_check(batch, stale_after)` — чистая SQL-функция поверх
         `find_duplicate_tasks`; алерт кладёт в `bot_notify` с `alert_type='duplicate'`
         и структурными полями (`duplicate_of_full_id`, `similarity`), окно
         подавления 2 часа (§5.1), `FOR UPDATE SKIP LOCKED`;
       · крон `duplicate-check-tick` `*/5 * * * *`, подтверждён прогон succeeded;
       · `p_stale_after` (default 1 час) закрывает старые джобы БЕЗ алерта — иначе
         48 джоб бэклога выплеснули бы в Telegram шквал. Бэклог слит: 0 pending;
       · ветка `duplicate` в `buildBroadcastCard` + `NotifyContext` (иначе алерт ушёл
         бы в `default` с текстом «Неизвестный тип»), 5 юнит-тестов в botNotifyCard.
       **Побочно найден и починен латентный баг (миграция 123):** `pg_trgm` живёт в
       схеме `extensions`, а `find_duplicate_tasks` — LANGUAGE sql без своего
       `search_path` → из любой функции с зафиксированным путём падал `42883:
       function similarity(text,text) does not exist`. Не проявлялось только
       из-за отсутствия потребителя. Пострадала ровно одна функция (проверено).
       Валидация: `type-check` ✅, `lint` 0 errors, тесты 38 файлов / 342 ✅;
       сквозной прогон воркера на реальных данных — `processed=1, no_match=1`;
       детект подтверждён на живых данных (similarity 0.929).
       Не сделано осознанно: реальный INSERT задачи-дубликата не производился
       (это отправило бы тестовое сообщение в рабочий Telegram) — путь проверен
       предикатно, алерты в проде ещё не появлялись.
