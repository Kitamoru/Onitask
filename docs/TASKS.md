...
- [ ] CAL-13 Раскрыть «аккаунт → календари»: probe displayname, затем миграция и UI #feature !high
      **В работе — зондирующий деплой (обязательно снести):** в `calendar-sync` добавлен
      `<D:displayname/>` в PROPFIND перечисления коллекций и блок `CALDAV_PROBE`, который пишет
      тело ответа в лог функции. Он нужен только чтобы выяснить, отдаёт ли Яндекс имена календарей
      и в каком написании prop-элемент. **После ответа блок удалить — он логирует тело ответа
      CalDAV в прод.**
      Далее по плану: сохранять источник события (`profile_calendars`, `calendar_events.calendar_id`,
      уникальность → `(calendar_id, remote_event_id)`); цвет = **учётка**, не календарь, поэтому
      `color_index` живёт в `calendar_connections`; палитра `--color-calendar-1..6` по кругу
      оттенков (в 4–5px-точках близкие оттенки неразличимы, поэтому `signal-yellow` и
      `accent-amber` не считаются двумя цветами); снять `UNIQUE (profile_id, provider)` и
      протащить `connection_id` через 5 мест, ищущих связь по профилю+провайдеру; UI — тап по
      пилюле открывает шторку (список календарей с галочками, затем «Синхронизировать» и
      «Удалить интеграцию» красным во втором шаге), рядом пилюля «+».
      Удаление интеграции = каскадное удаление её событий, без отмены (решено пользователем).
