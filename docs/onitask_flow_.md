...
- `is_blocked = true` → compact pure-red badge «Заблокирована» первым в badge-row `TaskCard` (без иконки и без дополнительного API-запроса); чистый красный намеренно отличается от кораллового приоритета «Высокий» и destructive-красного
...
**Эскалации:** список карточек с причиной + `suggested_action` + [Попробовать снова] + [Открыть задачу].
...
```
ALPHA-45 · Настроить pgvector-индексы
◆ Агент «Drift»
Агент остановился и ждёт вашего решения.
Причина: не удалось выполнить задачу после трёх попыток.
В ожидании 3 ч 14 мин

[Попробовать снова]  [Открыть задачу]
```

**[Попробовать снова]** подтверждается сообщением: агент начнёт новый проход,
история/комментарии/файлы сохранятся. Действие атомарно снимает эскалацию,
очищает retry-диагностику и ставит `dispatch_outbox(attempt=1)` назначенному
активному AI-агенту. Повторный клик безопасен: новый outbox не создаётся.
Заблокированная задача не допускает retry — сначала нужно проверить связанные
задачи. **[Открыть задачу]** использует существующий Task Sheet.

Пустое состояние: «Нет задач, ожидающих решения» / «Все задачи продолжают
...
**Две вкладки Task Sheet:** «Общее» и «Комментарии». Связи не вынесены в
отдельную вкладку.

Связи представлены постоянным блоком «Связанные задачи» внутри секции
«Дополнительный контекст» вкладки «Общее».
...
### Блок «Связанные задачи» (AGENT-04/09, A-12)

Отображается всегда (пустое состояние если нет рёбер), в том числе у завершённой
задачи; добавлять связи у `Сделано` нельзя. Источник данных — server-side
`GET /api/tasks/:id/relations` поверх `task_relations`. Instant tier, без LLM.
...
├─────────────────────────────────────────────────────────────────┤
│  ← Ждёт завершения:                                             │
│  ALPHA-34 · «Реализовать OAuth»                 Сделано         │
│                                                                 │
│  → После завершения этой задачи:                                │
│  ALPHA-48 · «Написать тесты для OAuth»         В очереди       │
│                                                                 │
│  [Добавить блокер]      [Эта задача блокирует]                   │
└─────────────────────────────────────────────────────────────────┘
```

**UX-правила блока «Связанные задачи»:**

Блок «← Ждёт завершения»:
- Показывается только при прямом `blocks`-ребре с `to_task_id = task.id`
- Карточка: `full_id` + title + системный статус с общей подписью
- Если блокер `Сделано`, а флаг текущей задачи остался stale — amber + «Исправить»
  → удалить ребро через `DELETE /api/tasks/:id/relations/:relationId`; RPC пересчитывает флаг
- Тап на карточку → открыть связанную задачу в Task Sheet с кнопкой «Назад»

Блок «→ После завершения этой задачи»:
- Показывается при прямом `blocks`-ребре с `from_task_id = task.id`
- Карточки downstream-задач: `full_id` + title + системный статус
- Тап → открыть связанную задачу с кнопкой «Назад»
...
**Действие:** RPC `delete_task_block_relation`; для orphan edge кнопка «Исправить»
удаляет связь, после чего автоматический пересчёт снимает stale `is_blocked`.
...
**Текущий UX supersedes v3.6.0:** отдельная вкладка «Блокировки» отменена.
Связи представлены постоянным блоком «Связанные задачи» в «Общем»; API —
`GET/POST/DELETE /api/tasks/:id/relations`, только `blocks`, с progress/impact,
orphan repair и навигацией «Назад» (ADR-2026-09-24).
