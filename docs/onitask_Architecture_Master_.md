...
В пользовательском интерфейсе и пользовательских метриках отключённый контур скрывается: `story_points_config.enabled=false` убирает SP, velocity и forecast; `enable_cognitive_budget=false` убирает F-01. A-11 остаётся отдельной метрикой.
Шкала SP по умолчанию — Фибоначчи `[1, 2, 3, 5, 8]`. `hours_per_sp` — необязательная team-specific стоимость; если она не задана, UI не выдумывает часы.
...
### 6.19 Календарь: calendar_events и напоминания

> Добавлен в v0.14.0. Реализует модуль «Календарь» (onitask_calendar_.md).
> Собственная интеграция на open-source клиентах: Yandex CalDAV (tsdav + ical.js),
> Outlook Graph API (@microsoft/microsoft-graph-client + @azure/msal-node).
...
-- ═══════════════════════════════════════════════════════
-- ТАБЛИЦА calendar_events
-- Хранит события из подключённых календарей workspace.
-- Каждый провайдер имеет свою запись в connection (см. ниже).
-- ═══════════════════════════════════════════════════════
...
CREATE TABLE calendar_events (
  ...
  -- Провайдер источника события.
  remote_event_id       text NOT NULL,
  -- Уникальный ID события в системе провайдера (для синхронизации).
  title                 text NOT NULL CHECK (char_length(title) BETWEEN 1 AND 500),
  description           text CHECK (char_length(description) <= 5000),
  start_at              timestamptz NOT NULL,
  end_at                timestamptz NOT NULL,
  -- end_at > start_at enforced триггером trg_validate_calendar_times.
  reminder_minutes_before int DEFAULT 15,
  -- NULL = без напоминания. Одно напоминание на событие (Phase 2: множественные).
  created_by            uuid REFERENCES workers(id) ON DELETE SET NULL,
  -- NULL = создано автоматически при синхронизации.
  updated_by            uuid REFERENCES workers(id) ON DELETE SET NULL,
  -- NULL = не редактировался вручную.
  source_synced_at      timestamptz,
  -- Последнее время синхронизации с провайдером.
  created_at            timestamptz NOT NULL DEFAULT now(),
  updated_at            timestamptz NOT NULL DEFAULT now(),
  UNIQUE (workspace_id, provider, remote_event_id)
  -- Один remote_event_id уникален в рамках workspace+provider.
);
...
CREATE INDEX idx_calendar_events_start_end
  ON calendar_events (workspace_id, start_at, end_at);

CREATE INDEX idx_calendar_events_provider
  ON calendar_events (workspace_id, provider);

CREATE INDEX idx_calendar_events_reminder_pending
  ON calendar_events (workspace_id, reminder_minutes_before, start_at)
  WHERE reminder_minutes_before IS NOT NULL;
  -- Частичный индекс: для быстрого поиска событий, требующих напоминания.


-- ═══════════════════════════════════════════════════════
-- ТАБЛИЦА calendar_connections
-- Хранит OAuth-токены подключённых календарных аккаунтов.
-- Токены хранятся зашифрованными (INV-17).
-- ═══════════════════════════════════════════════════════
...
CREATE TABLE calendar_connections (
  ...
  -- Владелец подключения (тот, кто авторизовал аккаунт).
  provider                text NOT NULL CHECK (provider IN ('yandex', 'outlook')),
  provider_account_email  text NOT NULL,
  -- Email аккаунта провайдера для отображения в UI.
  encrypted_oauth_tokens  bytea NOT NULL,
  -- Зашифрованные OAuth-токены (AES-256-GCM через pgcrypto).
  -- Ключ шифрования хранится в env var ENCRYPTION_KEY.
  token_expires_at        timestamptz,
  -- Время истечения токенов для проверки свежести.
  is_active               boolean NOT NULL DEFAULT true,
  connected_at            timestamptz NOT NULL DEFAULT now(),
  last_sync_at            timestamptz,
  UNIQUE (workspace_id, worker_id, provider)
  -- Один аккаунт провайдера на одного worker в workspace.
);

CREATE INDEX idx_calendar_connections_worker
  ON calendar_connections (worker_id);

CREATE INDEX idx_calendar_connections_workspace
  ON calendar_connections (workspace_id, is_active);


-- ═══════════════════════════════════════════════════════
-- ФУНКЦИЯ И ТРИГГЕРЫ: schedule_calendar_reminder
-- Планирует напоминание через enrichment_queue (type='bot_notify').
-- Снимает устаревшее напоминание при переносе времени или отмене.
-- ═══════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION schedule_calendar_reminder()
RETURNS TRIGGER AS $$
BEGIN
  -- Снимаем устаревшее напоминание при переносе времени или отмене
  DELETE FROM enrichment_queue
  ...
  IF NEW.reminder_minutes_before IS NOT NULL
     AND NEW.start_at - (NEW.reminder_minutes_before || ' minutes')::interval > NOW()
  THEN
    INSERT INTO enrichment_queue (
      workspace_id, type, payload, status, scheduled_at
    ) VALUES (
      NEW.workspace_id, 'bot_notify',
      jsonb_build_object(
        'workspace_id',     NEW.workspace_id,
        'alert_type',       'calendar_reminder',
        'event_id',         NEW.id,
        'target_worker_id', COALESCE(NEW.created_by, NEW.updated_by)
      ),
      'pending',
      NEW.start_at - (NEW.reminder_minutes_before || ' minutes')::interval
    );
  END IF;
  RETURN NEW;
END;
...
AFTER INSERT OR UPDATE OF start_at, reminder_minutes_before ON calendar_events
FOR EACH ROW EXECUTE FUNCTION schedule_calendar_reminder();


-- ═══════════════════════════════════════════════════════
-- ФУНКЦИЯ И ТРИГГЕР: cancel_calendar_reminder
-- Удаляет pending reminder при удалении события.
-- ═══════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION cancel_calendar_reminder()
RETURNS TRIGGER AS $$
BEGIN
  DELETE FROM enrichment_queue
  WHERE type = 'bot_notify'
...
FOR EACH ROW EXECUTE FUNCTION cancel_calendar_reminder();


-- ═══════════════════════════════════════════════════════
-- ВАЛИДАЦИЯ: trg_validate_calendar_times
-- end_at > start_at enforced.
-- ═══════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION validate_calendar_times()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW EXECUTE FUNCTION validate_calendar_times();


-- ═══════════════════════════════════════════════════════
-- INV-17: Шифрование OAuth-токенов
-- ═══════════════════════════════════════════════════════
-- calendar_connections.encrypted_oauth_tokens хранится в зашифрованном виде.
-- Ключ шифрования: env var ENCRYPTION_KEY (AES-256-GCM через pgcrypto).
-- Расшифровка возможна только внутри БД через service function.
-- Внешние API вызываются ТОЛЬКО из Edge Functions, токен никогда не передаётся клиенту.
...
|------|-----------|---------------|----------|
| `bot_notify` | `calendar_reminder` | `workspace_id`, `event_id`, `target_worker_id` | Напоминание о событии календаря |

#### Маршрутизация напоминаний

- Если `target_worker_id` присутствует → воркер резолвит `workers.source_id → profiles.telegram_id` и шлёт `sendMessage` напрямую в личный чат (`chat_id = telegram_id`).
- Edge case: пользователь никогда не писал боту (`/start` не было) → Telegram вернёт 403, DM невозможен. Обрабатываем как `failed`, без ретраев.
- Если `target_worker_id` отсутствует → broadcast во все привязанные `workspace_telegram_chats` (текущее поведение Bot Notify Worker).

#### Хранение данных

| Таблица | Срок хранения | Действие |
|---|---|---|
| `calendar_events` | бессрочно (в рамках workspace) | CASCADE при удалении workspace |
| `calendar_connections` | до явного удаления Admin/Owner или worker | CASCADE при удалении workspace/worker |
...
*Модуль «Календарь»:*
- §6.19 (новый): таблицы `calendar_events` и `calendar_connections`; триггеры
  `trg_schedule_calendar_reminder`, `trg_cancel_calendar_reminder`, `trg_validate_calendar_times`;
  INV-17 (шифрование OAuth-токенов); типы событий в enrichment_queue; маршрутизация напоминаний
  через `target_worker_id`; политика хранения
- §9: добавлены `calendar_events` и `calendar_connections` в таблицу хранения данных
- §1: INV-17 (шифрование токенов calendar_connections) добавлен в список инвариантов
