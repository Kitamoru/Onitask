...
export function DocumentsCard({
  ...
  return (
    <Card>
      ...
      {/*
        Previously the "Выберите файл" field displayed only the most
        recently picked file's name — the underlying state already
        supported up to 10 files (multiple + slice(0,10)), but nothing
        in the UI showed more than one at a time, so attaching several
        files looked like it silently only kept the last one. Fixed by
        rendering the full list here, each row individually removable —
        same pattern as the external-links list below in this form.
      */}
      {files.length > 0 && (
      ...
      >
        ...
        >
          {/* Static label, not the last filename — the list above is
              now the single source of truth for what's attached. */}
