...
export function Stepper({
  ...
  /** [from, to] accent gradient — e.g. amber for the warning threshold,
   *  red for the urgent one, matching the traffic-light semantics. */
...
function StepperButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "flex h-8 w-8 items-center justify-center rounded-full text-text transition-opacity",
        disabled && "opacity-30"
      )}
    >
      {children}
