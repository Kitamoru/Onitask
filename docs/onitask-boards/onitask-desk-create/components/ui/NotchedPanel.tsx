...
type NotchedPanelProps = {
  ...
  children?: ReactNode;
  /**
   * Zero out the bottom-left/bottom-right radius, keeping only the top
   * two corners rounded (before the chamfer clip further cuts them).
   * For `corner="sheet"`: a bottom sheet sits flush against the
   * device's own bottom edge — a uniform radius there would round the
   * bottom corners against nothing, reading as a stray visual nub right
   * at the screen edge instead of a seamless attachment to it.
   */
...
export function NotchedPanel({
  ...
  const innerRadiusValue = Math.max(radius - borderWidth, 0);

  const outerRadius = squareBottom ? `${radius}px ${radius}px 0 0` : radius;
  const innerRadius = squareBottom
    ? `${innerRadiusValue}px ${innerRadiusValue}px 0 0`
    : innerRadiusValue;
