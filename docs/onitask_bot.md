...
### 2.1 Persistent Reply-клавиатура

В личном чате после `/start` бот отправляет компактную постоянную Reply-клавиатуру с уже реализованными командами:
...
Кнопки отправляют обычный текст команды, поэтому обрабатываются тем же webhook command pipeline без отдельных handlers. Клавиатура имеет `resize_keyboard=true`, `is_persistent=true`, `one_time_keyboard=false`; при скрытии Telegram показывает штатную кнопку восстановления клавиатуры. В групповых чатах Reply-клавиатура не отправляется, чтобы не менять интерфейс всех участников. Нереализованные команды (`/inbox`, `/flow`, `/standup`, `/summary`, `/resolve`) в кнопки не включены.
