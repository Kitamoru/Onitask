...
`var(--color-signal-green, #4ADE80)`. Ранее открытый спринт использовал общий
`var(--color-accent)`, который в текущей теме равен amber. Геометрия, проценты,
track и состояния не менялись. Подпись количества завершённых задач в компактной
статистике спринта унифицирована с системным статусом: `Готово:` → `Сделано:`.

---

## AGENT-04/09 · «Связанные задачи» + badge блокировки (2026-09-24) ✅

**Дополнение:** общий `TaskCard` при `tasks.is_blocked=true` первым badge
показывает compact pure-red бейдж «Заблокирована» без иконки. Он используется в
`ColumnTasksSheet`, Stream и других карточках на базе общего `TaskCard`; порядок
задач, API и БД не меняются. Цвет `#FF0000` намеренно отличается от кораллового
приоритета «Высокий» (`#EF4444`) и destructive-красного (`#FF2B3A`). В production
на момент проверки: 2 blocked tasks, 3 `blocks` edges.
...
## 2026-09-26 — Календарь: UI, часовые пояса, безопасность (итог пула)

Закрыт весь техдолг и незавершённые пункты по календарному пулу. Все коммиты в `origin/main`
(`508ad83` … `d0b6878` + правки иконок). Прод: Edge Function `calendar-sync` v42.

### Что сделано

**Часовой пояс (главный баг, был в проде).** `DTSTART;TZID=Europe/Moscow:...` — параметры
свойства отбрасывались регуляркой, «плавающее» время читалось в локальной зоне рантайма (UTC на
Edge Functions), поэтому событие на 10:00 по Москве хранилось как 10:00 UTC и показывалось на
три часа позже. Плюс ключ дня везде считался по UTC, при этом час рисовался по-локальному.
Исправлено в `calendar-sync/ical.ts` + `localDateKey` в `lib/calendar`.
Разбор вынесен в отдельный модуль, чтобы его можно было тестировать (у `index.ts` Deno с
`@ts-nocheck` и URL-импортами, vitest его не берёт).

**Безопасность — дыра была реальной.** Guard Edge Function дополнительно принимал любой
`Bearer ` длиннее 10 символов, а секрет `SUPABASE_SERVICE_KEY`, с которым он сравнивал, в
проекте **не существует** — так что первая ветка всегда падала и запрос уходил во вторую.
Функция была фактически публичной: anon-ключ из клиентского бандла её удовлетворял. Проверено
живым запросом до и после: поддельный bearer, публичный anon-ключ и отсутствие заголовка — теперь
все три дают 401. `disconnect/[provider]` не аутентифицировал вовсе и брал `profile_id` из тела
(IDOR) — теперь `authenticateRequest` + `auth.profileId` + service-ключ. OAuth `state` был сырым
`profile_id`, что позволяло подделать callback и записать свой календарь в чужой аккаунт — теперь
короткоживущий HMAC (`lib/calendar-oauth-state.ts`).

**Экран.** Вместо мёртвого react-day-picker — `День` / `Месяц` (высота свёрнутого BottomSheet не
вмещает переключатель на 4 режима), `WeekStrip` для переключения дней, пропорциональная часовая
ось с блоками по реальной длительности и колонками для пересечений, `MonthView` с читаемыми
точками. Удалены `CalendarView`, `ListView`, `ThreeDaysView`, `MonthListView` и 3 неиспользуемых
режима. Тап по событию наконец делает что-то (`EventDetailSheet` на штатной `BottomSheet`).
Автосинхронизация при открытии с порогом протухания 15 минут — синк это запись во внешнюю
систему (~2.7 с, 18 запросов), поэтому `refetchInterval` здесь неуместен.

**События на весь день.** `VALUE=DATE` хранились как полночь UTC и попадали на ось как 03:00.
Миграция 124 добавила `calendar_events.is_all_day`; парсер метит флаг, `AllDayRow` показывает их
над осью, карточка — без диапазона времени.

### Проверка

`tsc` чист (единственная ошибка — предавняя в `botNotifyCard.test.ts`), ESLint 0 ошибок,
**42 файла / 410 тестов**, сборка `Compiled successfully`.

### Уроки этого пула (важно для будущих задач)

1. **Мутационная проверка нужна, иначе тест может ничего не проверять.** Первая версия тестов на
   iCal проходила **с удалённой обработкой TZID**: рантайм был в Europe/Moscow, поэтому ошибочный
   фолбэк в локальное время давал тот же инстант, что и правильная конверсия. Ожидания переписаны
   через сам `timeZoneOffsetMs`, и каждая зона теперь дополнительно проверяется утверждением
   «передача зоны меняет результат».
2. **Тестов рендера в проекте нет вообще** (`environment: 'node'`, `include: '*.test.ts'`), и
   именно поэтому два бага (забытый `grid-cols-7`, неограниченная высота корня) прошли мимо всего,
   чем я располагал. См. открытый вопрос про `jsdom` + `@testing-library/react`.
3. **Не использовать `Get-Content` / `Set-Content` для правок** — Windows PowerShell 5.1 читает в
   ANSI, и кириллица в `page.tsx` была уничтожена двойным кодированием. Только
   `[System.IO.File]::ReadAllText` / `WriteAllText`. Обратные кавычки в строках PowerShell — символ
   экранирования, из-за чего ломались и поисковые шаблоны.

### Открыто

- Тесты рендера компонентов и страницы (нужно решение по инфраструктуре: jsdom + testing-library).
- `docs/TASKS.md` не обновлялся в этом пуле.
- Визуальная проверка всего нового UI невозможна из агента — всё, что связано с вёрсткой,
  подтверждается только вручную в Mini App.
