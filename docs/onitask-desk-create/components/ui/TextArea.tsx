...
export const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(
  function TextArea(
    ...
    return (
      // NotchedPanel's outer/inner layers are both "h-full" — with no
      // ancestor here setting an explicit height, that resolves to
      // "auto" per the CSS spec (percentage height on an auto-height
      // ancestor computes as auto), so the panel just hugs whatever
      // height the textarea below ends up at. Same box-model mechanic
      // that made the earlier Button height-chain fix work, just run
      // in the "let it grow" direction instead of "force a fixed one".
