...
export type CornerStyle = "panel" | "action" | "field" | "none" | "sheet";
...
export function clipPathFor(style: CornerStyle, notch: number): string | undefined {
  ...
  switch (style) {
    ...
    case "sheet":
      // cut: top-left + top-right, bottom stays a plain flush edge —
      // this is the shape of a sheet sliding up from the device's own
      // bottom edge, so there's nothing there to round or cut.
      return `polygon(${n} 0, calc(100% - ${n}) 0, 100% ${n}, 100% 100%, 0 100%, 0 ${n})`;
    case "none":
    default:
      return undefined;
  }
