...
--
-- Drops: label, autonomy_level, playbook_variant (all inert after Stage 5)
-- Backfills: agent_name from label for active keys
-- ============================================================================

-- Backfill agent_name from label for keys that have empty agent_name
UPDATE mcp_agent_keys
SET agent_name = label
WHERE agent_name = '''' AND label IS NOT NULL AND label != '''';
...
-- Comments (cleanup)
COMMENT ON COLUMN mcp_agent_keys.agent_name IS ''Agent identity (1 key = 1 agent). Shown as "азвание ключа" in UI.'';
