...
-- Semantics (CTX-01a, two-phase delivery): запись delivered ставится в момент
--   доставки задачи в ответе; клиент подтверждает обработку, возвращая
--   обработанные id через known_task_ids (ack). TTL записей (delivered ~10 мин,
--   acked ~24ч) и cap размера поддерживаются кодом
