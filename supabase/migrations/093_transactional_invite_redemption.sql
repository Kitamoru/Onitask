-- ============================================================================
-- 093_transactional_invite_redemption.sql
-- WS-06: transactional invite redemption.
-- The worker membership change and invite use increment commit together.
-- Repeating the same link for an already-active member is idempotent.
...
BEGIN
  -- Serialize redemptions of one link and re-check quota under the row lock.
...
-- The legacy one-argument overload is no longer called by the application.
-- Remove its unsafe authenticated grant and direct public execution.
REVOKE ALL ON FUNCTION public.accept_invite_link(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.accept_invite_link(text) FROM anon;
REVOKE ALL ON FUNCTION public.accept_invite_link(text) FROM authenticated;
